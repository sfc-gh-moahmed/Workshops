...
FROM (
    ...
    -- DS_PHARMACY_ALLMEDICALSCRIPTS — winner: FIRST_DELIVERYDATE (verified: 9,986 last-12m rows; RXSTARTDATE 71% null)
    SELECT 'DS_PHARMACY_ALLMEDICALSCRIPTS' AS tbl,
           'FIRST_DELIVERYDATE'            AS col,
           COUNT(*)                        AS total_rows,
           SUM(CASE WHEN FIRST_DELIVERYDATE IS NULL THEN 1 ELSE 0 END) AS null_rows,
           MIN(FIRST_DELIVERYDATE)         AS min_date,
           MAX(FIRST_DELIVERYDATE)         AS max_date,
    ...
    -- DS_PHARMACY_ALLMEDICALSCRIPTS — alternate: RXSTARTDATE (71% null — do not use as filter)
    ...
    -- DS_PHARMACY_DISPENSINGHISTORY — winner: DISPENSINGDATEOFSERVICE (verified col 15)
    SELECT 'DS_PHARMACY_DISPENSINGHISTORY',
           'DISPENSINGDATEOFSERVICE',
           COUNT(*),
           SUM(CASE WHEN DISPENSINGDATEOFSERVICE IS NULL THEN 1 ELSE 0 END),
           MIN(DISPENSINGDATEOFSERVICE), MAX(DISPENSINGDATEOFSERVICE),
           COUNT(CASE WHEN DISPENSINGDATEOFSERVICE >= DATEADD('month', -12, CURRENT_DATE())
                       THEN 1 END)
    FROM PROD.LAKE_HDMS.DS_PHARMACY_DISPENSINGHISTORY

    UNION ALL

    -- DS_PHARMACY_DISPENSINGHISTORY — alternate: PHARMACYDRUGFILLDATE (verified col 5)
    SELECT 'DS_PHARMACY_DISPENSINGHISTORY',
           'PHARMACYDRUGFILLDATE',
           COUNT(*),
           SUM(CASE WHEN PHARMACYDRUGFILLDATE IS NULL THEN 1 ELSE 0 END),
           MIN(PHARMACYDRUGFILLDATE), MAX(PHARMACYDRUGFILLDATE),
           COUNT(CASE WHEN PHARMACYDRUGFILLDATE >= DATEADD('month', -12, CURRENT_DATE())
                       THEN 1 END)
    FROM PROD.LAKE_HDMS.DS_PHARMACY_DISPENSINGHISTORY

    UNION ALL

    -- MEDICATION_ORDER_ALL — winner: MEDICATION_ORDER_CREATE_DATE (5M last-12m rows, 0% null)
    ...
    -- MEDICATION_ORDER_ALL — alternate: ENCOUNTER_DATE (junk dates 1901-2099 — do not use)
    SELECT 'MEDICATION_ORDER_ALL',
           'ENCOUNTER_DATE',
           COUNT(*),
...
FROM (
    ...
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN a.FIRST_DELIVERYDATE >= DATEADD('month', -12, CURRENT_DATE())
...
--
-- Example for DISPENSINGHISTORY (replace date-based filter with this):
--
...
--       SELECT SCRIPTNUMBER
--       FROM PROD.LAKE_HDMS.DS_PHARMACY_ALLMEDICALSCRIPTS
--       WHERE FIRST_DELIVERYDATE >= DATEADD('month', -12, CURRENT_DATE())
--   )
--   LIMIT 50000;
--
